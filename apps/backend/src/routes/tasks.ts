...
// - tasks(status, deadline) - for overdue/due soon filtering
// - tasks(projectId, status) - for project-based filtering
// - tasks(priority, createdAt) - for priority sorting
// - tasks(deadline) - for date-based sorting
// - tasks(tags) - for tag-based filtering (if using full-text search)
router.get('/', async (req, res) => {
  try {
    ...
    // Date-based filters - handle conflicts properly
    const dateFilters: any[] = [];
    const statusFilter = { not: 'done' }; // Common status filter for both overdue and due soon
    
    // Overdue filter
    if (overdue === 'true') {
      dateFilters.push({ deadline: { lt: now } });
    }

    // Due soon filter (next 7 days)
    if (dueSoon === 'true') {
      const nextWeek = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000);
      dateFilters.push({ deadline: { gte: now, lte: nextWeek } });
    }
    
    // Apply date filters properly - use OR when multiple date filters exist
    if (dateFilters.length > 0) {
      if (dateFilters.length === 1) {
        // Single date filter
        Object.assign(where, dateFilters[0]);
      } else {
        // Multiple date filters - use OR logic
        where.OR = where.OR ? [...where.OR, ...dateFilters] : dateFilters;
      }
      // Apply status filter for date-based filters
    ...
    // Optimize queries by selecting only necessary fields to reduce data transfer
    const [tasks, total] = await Promise.all([
      prisma.task.findMany({
        ...
        select: {
          ...
          actualHours: true,
          tags: true,
          // Only select essential fields from related entities
          project: {
